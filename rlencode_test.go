...
func TestOne(t *testing.T) {
	...
	for _, table := range tables {
		...
		count++
		value2 := RunLengthDecode(value)
		if !equal(value2, table.testCase) {
			t.Errorf("RLE %s -> %s was incorrect, expected: %s.", ArrayAsString(value), ArrayAsString(value2), ArrayAsString(table.testCase))
		} else {
			t.Logf("RLE %s was correct, expected: %s.", ArrayAsString(value2), ArrayAsString(table.testCase))
			successCount++
		}
		count++
	}
